// ============================================================
// modules/timetable/timetableRoomView.js
// Room-wise Timetable View
// Mirror of timetableTeacherView.js:
//  - Rows     → Rooms  (instead of Teachers)
//  - Cell     → Subject | Batch No. | Teacher  | Time
//  - Export   → CSV + PDF  (same pattern)
// ============================================================

import { AppState } from '../../utils/state.js';
import { getAssignmentForBatch } from '../lecturePlan/lecturePlanService.js';

export const TimetableRoomView = (() => {
  ...
  function _buildSlots(entries) {
    ...
    entries.forEach(e=>{
      const s=_toMins(e.startTime), n=_toMins(e.endTime);
      if(s<mn) mn=s; if(n>mx) mx=n;
    });
  ...
  // 16-slot palette ordered so every adjacent pair is a completely different hue family.
  // Blue→Orange→Green→Pink→Teal→Yellow→Purple→Red→Indigo→Emerald→Amber→Violet→Cyan→Rose→DeepGreen→DeepAmber
  const PALETTE = [
  ...
  // FNV-1a hash — excellent bit distribution for short strings like group codes.
  // Same groupCode always maps to the same palette slot, regardless of what
  // other groups happen to be present. Export uses this same map so colors
  // are identical on screen and in PDF/CSV output.
  ...
  // ── Collect all timetable entries with room info ──────────
    ...
    timetables.forEach(tt => {
      ...
      (tt.groups || []).forEach(grp => {
        (grp.subjects || []).forEach(sub => {
          // Must have a room and times
          if (!sub.roomId || !sub.startTime || !sub.endTime) return;
          ...
          // Skip expired batches — room should be free
  ...
  function _filter(entries) {
    return entries.filter(e => {
      ...
      if (_filterRoom   && e.roomId   !== _filterRoom)   return false;
      return true;
    });
  }

  // Total hours a room is occupied on a given day
  function _totalHrsDay(roomEntries, day) {
    const seen = new Set(); let mins = 0;
    roomEntries.filter(e => e.days.includes(day)).forEach(e => {
  ...
  // ── Styles — identical to teacher view ───────────────────
  const STYLES = `
  <style id="ttvRoomStyles">
    ...
    .ttv-tbl th.th-name{text-align:left;min-width:130px;position:sticky;left:0;top:0;z-index:5;background:var(--surface2);border-right:2px solid var(--border);}
  ...
  function _render() {
    ...
    // Unique room IDs present in filtered entries (preserve natural room order)
    const allRoomsSorted = rooms
      .filter(r => filtered.some(e => e.roomId === r.id))
      .sort((a,b) => {
        const numA = parseInt((a.name||'').replace(/\D/g,'')) || 0;
        const numB = parseInt((b.name||'').replace(/\D/g,'')) || 0;
        return numA !== numB ? numA - numB : (a.name||'').localeCompare(b.name||'');
      });
    const roomIds = allRoomsSorted.map(r => r.id);

    // Rooms for filter dropdown (only those with entries)
    const roomsWithEntries = rooms.filter(r => allEntries.some(e => e.roomId === r.id))
      .sort((a,b)=>{
    ...
    <div class="ttv-wrap">
      <div class="ttv-toolbar">
        ...
        <select class="ttv-sel" id="ttvRoomCampus">
        ...
        <select class="ttv-sel" id="ttvRoomDisc">
          <option value="">All Disciplines</option>
          ${discs.map(d=>`<option value="${_esc(d.id)}"${_filterDisc===d.id?' selected':''}>${_esc(d.abbreviation||d.name||d.id)}</option>`).join('')}
        </select>
        <span class="ttv-lbl">Room</span>
        <select class="ttv-sel" id="ttvRoomFilter">
          <option value="">All Rooms</option>
          ${roomsWithEntries.map(r=>`<option value="${_esc(r.id)}"${_filterRoom===r.id?' selected':''}>${_esc(r.name)}</option>`).join('')}
        </select>
        <div class="ttv-export-grp">
          <button class="ttv-icon-btn" id="ttvRoomExportCSV" title="Export CSV">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <rect x="3" y="3" width="18" height="18" rx="2"/><path d="M8 12l2.5 2.5L16 9"/>
            </svg>
          </button>
          <button class="ttv-icon-btn" id="ttvRoomExportPDF" title="Export PDF">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="ttv-day-pills" id="ttvRoomPills">
        ${ALL_DAYS.map(d => {
      ...
      <div class="ttv-scroll">
        ${roomIds.length === 0
          ? `<div class="ttv-empty">
               ...
               <div><h3>No Rooms Found</h3><p>Add timetable entries with room assignments to see the schedule here.</p></div>
             </div>`
          : `<table class="ttv-tbl">
               <thead><tr>
               ...
               <tbody>
                 ${roomIds.map(rid => _roomRow(rid, slots, dayEntries, filtered, day, colorMap)).join('')}
               </tbody>
             </table>`
        }
      </div>
    ...
    _rootEl.querySelector('#ttvRoomCampus')?.addEventListener('change', e => { _filterCampus = e.target.value; _render(); });
    _rootEl.querySelector('#ttvRoomDisc')?.addEventListener('change',   e => { _filterDisc   = e.target.value; _render(); });
    _rootEl.querySelector('#ttvRoomFilter')?.addEventListener('change', e => { _filterRoom   = e.target.value; _render(); });
    _rootEl.querySelector('#ttvRoomPills')?.addEventListener('click', e => {
      const p = e.target.closest('.ttv-dp'); if (!p) return;
      _rootEl._dayFilter = p.dataset.day; _render();
    });
    _rootEl.querySelector('#ttvRoomExportCSV')?.addEventListener('click', () => {
      _exportCSV(filtered, day, slots, roomIds, colorMap);
    });
    _rootEl.querySelector('#ttvRoomExportPDF')?.addEventListener('click', () => {
      _exportPDF(filtered, day, slots, roomIds, colorMap);
    });
  }

  // ── Room row (screen) ─────────────────────────────────────
  // Cell content: Subject | Batch No. | Teacher name | Time
  function _roomRow(rid, slots, dayEntries, allFiltered, day, colorMap) {
    const rde  = dayEntries.filter(e => e.roomId === rid);
    const rae  = allFiltered.filter(e => e.roomId === rid);
    const name = rae[0]?.roomName || rid;
    const totalHrs = _totalHrsDay(rae, day);

    const cells = []; let skip = 0;
    for (let i = 0; i < slots.length; i++) {
      if (skip > 0) { cells.push({skip:true}); skip--; continue; }
      const ss    = slots[i];
      const entry = rde.find(e => _toMins(e.startTime) === ss);
      if (entry) {
    ...
    const tds = cells.map(c => {
      ...
      return `<td class="td-slot" colspan="${c.span}">
        <div class="ttv-cell" style="background:${col.bg};border-color:${col.bd};color:${col.tx}">
          ...
          <span class="cr">${_esc(e.teacherName)}</span>
  ...
  function _buildExportHTML(filtered, day, slots, roomIds, colorMap, mode) {
    const now     = new Date();
    ...
    if (_filterRoom) {
      const r = (AppState.get('rooms')||[]).find(x => x.id === _filterRoom);
      if (r) labelParts.push(`Room: ${r.name}`);
    }
    const filterHTML = labelParts.length
      ? labelParts.map(f => `<span class="filter-chip">${f}</span>`).join('')
      : '<span class="filter-chip no-filter">No filters — showing all rooms</span>';
    ...
    // Tbody
    const tbodyRows = roomIds.map(rid => {
      const rde  = dayEntries.filter(e => e.roomId === rid);
      const rae  = filtered.filter(e => e.roomId === rid);
      const name = rae[0]?.roomName || rid;
      const totalHrs = _totalHrsDay(rae, day);
    ...
    const csvDataJson = JSON.stringify(
      roomIds.flatMap(rid => {
        const rde      = dayEntries.filter(e => e.roomId === rid);
        const rae      = filtered.filter(e => e.roomId === rid);
        const roomName = rae[0]?.roomName || rid;
        const totalHrs = _totalHrsDay(rae, day);
        return rde.map(e => [
          roomName,
          e.subjectCode,
      ...
      ? `<div class="no-print" style="margin-top:18px;text-align:center;display:flex;align-items:center;justify-content:center;gap:12px">
         ...
         <script>
           (function(){
             ...
             var hdr=['Room','Subject','Batch No.','Batch Name','Teacher','Start','End','Duration(hrs)','Day','Total Hrs'];
             document.getElementById('csvDlBtn').onclick=function(){
               ...
               a.href=url;
               a.download='Room-Timetable-${dateStr.replace(/ /g,'-')}.csv';
...
<head>
<meta charset="UTF-8"/>
<title>Room Timetable — ${day}</title>
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  thead th.th-name{text-align:left;min-width:110px;border-right:2px solid rgba(255,255,255,.3)}
  ...
  .ttv-cell{border-radius:7px;padding:6px 8px;line-height:1.45;display:flex;flex-direction:column;gap:1px;min-height:56px;justify-content:center;border-width:1px;border-style:solid;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-variant-numeric:normal;font-feature-settings:'zero' 0;}
...
<body>
  <div class="header">
    <div class="header-left">
      <div class="title">Room Timetable Report</div>
      <div class="subtitle">Room-wise Weekly Schedule</div>
    </div>
  ...
  <div class="meta-row">
    ...
    <div class="stat-box"><div class="num">${roomIds.length}</div><div class="lbl">Rooms</div></div>
  ...
  <div class="footer">
    <span>Room Timetable &nbsp;|&nbsp; Exported on ${dateStr} at ${timeStr}</span>
    <span>${roomIds.length} room${roomIds.length!==1?'s':''} &nbsp;|&nbsp; ${day}</span>
  </div>
  ...
  // ── CSV Export ────────────────────────────────────────────
  function _exportCSV(filtered, day, slots, roomIds, colorMap) {
    if (!filtered.length) { alert('No data to export.'); return; }
    const html = _buildExportHTML(filtered, day, slots, roomIds, colorMap, 'csv');
  ...
  // ── PDF Export ────────────────────────────────────────────
  function _exportPDF(filtered, day, slots, roomIds, colorMap) {
    if (!filtered.length) { alert('No data to export.'); return; }
    const html = _buildExportHTML(filtered, day, slots, roomIds, colorMap, 'pdf');
    const w = window.open('', '_blank');
    w.document.write(html);
    w.document.close();
  ...
  function _bindSync() {
    ...
    if (typeof AppState.set === 'function' && !AppState._ttRoomViewPatched) {
      ...
      AppState._ttRoomViewPatched = true;
    }
  }
