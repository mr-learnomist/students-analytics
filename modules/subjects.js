...
function _bi_findSubjectByCode(code) {
  ...
  // Active subject preferred; fall back to archived so historic CSV imports
  // continue to work even after a subject has been archived.
  return subjects.find(s => (s.subjectCode || '').toLowerCase() === needle && !s.isArchived)
      || subjects.find(s => (s.subjectCode || '').toLowerCase() === needle)
      || null;
}
...
export const BatchModule = {
  ...
  _openForm(existing = null, container) {
    ...
    const subjectOptions = (() => {
      // Use getSelectableSubjects: active only for new batches; for edits,
      // always includes the currently-saved subject even if archived.
      const liveOpts = getSelectableSubjects(selLvlId, _frozenSubjId)
        .map(s => {
          ...
          if (isEdit && s.id === _frozenSubjId) {
            ...
            return `<option value="${s.id}" selected>${frozenLabel}${s.isArchived ? ' [archived]' : ''}</option>`;
          }
          return `<option value="${s.id}">${s.subjectCode} — ${s.subjectName}${s.isArchived ? ' [archived]' : ''}</option>`;
        }).join('');
      // If editing and subject exists but wasn't found in live list
      // (deleted subject edge case), still show frozen label as selected
      if (isEdit && _frozenSubjId && !AppState.get('subjects')?.find(s => s.id === _frozenSubjId)) {
  ...
  _wireDynamicDropdowns(modalEl, existing = null) {
    ...
    levelSel?.addEventListener('change', () => {
      // Active subjects only when user picks a new level (no currentSubjectId
      // context here — they are choosing fresh).
      const subs = getSelectableSubjects(levelSel.value);
