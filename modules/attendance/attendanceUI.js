...
// ══════════════════════════════════════════════════════════════
// IMPORT / EXPORT MODULE  (patched — fixes: student list, date
//   format, per-batch CSV, import parser, attMap lookup)
// ══════════════════════════════════════════════════════════════
...
// ── Helper: "Mon 11-Mar" (for column header) ──────────────────
...
// ── Helper: enrolled active students for a batch ──────────────
...
// ── Helper: attendance records → { studentId: { date: status } }
...
// ── Helper: class dates for batch (optionally filtered by range)
function _getBatchClassDates(batch, from, to) {
  const lpaMap = AppState.get('lpAssignments') || {};
  const lpa    = lpaMap[batch.id];
  let dates = [];
  if (lpa && lpa.rows && lpa.rows.length) {
    ...
    <div style="flex:1;overflow-y:auto;padding:24px;display:flex;flex-direction:column;gap:20px;max-width:860px;margin:0 auto;width:100%">
      ...
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:22px">
        ...
        <div style="display:flex;gap:10px;margin-top:12px;flex-wrap:wrap">
          ${[
            ['P = Present',  'var(--green)', 'var(--green-dim)'],
            ['A = Absent',   'var(--red)',   'var(--red-dim)'  ],
            ['L = Leave',    '#92400e',      '#fef3c7'         ],
            ['blank = no data', 'var(--t3)', 'var(--surface2)' ],
          ].map(([label, color, bg]) =>
...
function _attachImportExportEvents() {
  const updateCount = () => {
    ...
    const count = (AppState.get('batches') || []).filter(b => {
      ...
      return _getBatchClassDates(b, from, to).length > 0 &&
             _getBatchStudents(b.id).length > 0;
    }).length;
...
function _parseAndPreviewImport(lines) {
  ...
  for (const row of lines) {
    ...
    if (cell0.startsWith('Fill:') || cell0.startsWith('Exported:')) continue;

    // Column header row
    if (cell0 === '#') {
      classDates = row.slice(3)
        .filter(h => h && !['Total P','Total A','Leave','Attendance %'].includes(h))
        .map(h => { const m = h.match(/(\d{4}-\d{2}-\d{2})/); return m ? m[1] : null; })
        .filter(Boolean);
      inStudents = true;
      continue;
    }
    ...
    if (!stuName && !stuReg) continue;

    // Match student: reg/id first, then name
    ...
    if (!student) {
      if (stuName) errors.push(`Could not find student: "${stuName}" (${stuReg})`);
      continue;
    }
    ...
    classDates.forEach((date, i) => {
      ...
      const status = ['P','A','L'].includes(raw) ? raw : null;
      if (!status) return;
      const existing = (attMap[student.id] || {})[date];
      if (existing) {
        skipped.push({ batch: currentBatch.batchName, student: student.studentName || stuName, date: _fmtDate(date), existing });
      } else {
        updates.push({ batchId: currentBatch.id, studentId: student.id, date, status, batchName: currentBatch.batchName, studentName: student.studentName || stuName });
      }
    });
  }
    ...
    <div style="border:1px solid var(--border);border-radius:10px;overflow:hidden;margin-top:8px">
      ...
      ${updates.length === 0 ? `
        <div style="padding:24px;text-align:center;color:var(--t3);font-size:13px">
          No new records to import. All data already marked or no matching students found.
        </div>` : `
              ...
              ${updates.slice(0,60).map(u => `
                <tr style="border-bottom:1px solid var(--border)">
                  ...
                  <td style="padding:7px 12px;text-align:center">
                    <span style="font-size:11px;font-weight:700;padding:2px 10px;border-radius:6px;background:${statusBg(u.status)};color:${statusColor(u.status)}">${statusLabel(u.status)}</span>
                  </td>
                </tr>`).join('')}
              ${updates.length > 60 ? `<tr><td colspan="4" style="padding:8px 12px;text-align:center;color:var(--t3);font-size:11px">...and ${updates.length - 60} more</td></tr>` : ''}
            </tbody>
          </table>
        </div>
        <div style="padding:14px 18px;display:flex;align-items:center;gap:10px;border-top:1px solid var(--border);flex-wrap:wrap">
          ...
          <button id="cancelImportBtn" style="padding:9px 16px;background:var(--surface2);color:var(--t2);border:1px solid var(--border2);border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit">Cancel</button>
  ...
  _root.querySelector('#confirmImportBtn')?.addEventListener('click', () => {
    ...
    updates.forEach(u => {
      try { AttendanceService.markAttendance(u.batchId, u.studentId, u.date, u.status, markedBy); saved++; }
      catch(err) { console.warn('Import error:', err); }
    });
    AppState.saveState();
    Toast.success(`✅ ${saved} records imported! ${skipped.length} skipped.`);
    preview.innerHTML = `
      <div style="padding:24px;text-align:center;background:var(--green-dim);border:1px solid var(--green);border-radius:10px;margin-top:8px">
        <div style="font-size:18px;font-weight:800;color:var(--green);margin-bottom:4px">✅ Import Complete!</div>
        <div style="font-size:13px;color:var(--t2)">${saved} records saved · ${skipped.length} skipped · ${errors.length} unmatched</div>
      </div>`;
  });
