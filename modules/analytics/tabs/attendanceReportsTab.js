...
// Shows batch-wise P/A/L per student across all dates
// ══════════════════════════════════════════════════════════════
function _renderAttendanceSheet(container, onBack) {
  ...
  // ── Back button ────────────────────────────────────────────
  container.querySelector('#attRepBack').addEventListener('click', onBack);
  ...
  const _populateBatches = () => {
    ...
    sel.innerHTML = `<option value="">— Select Batch —</option>` +
      filtered.map(b => `<option value="${b.id}" ${b.id === prev ? 'selected' : ''}>${b.batchName}</option>`).join('');
    _batchId = sel.value;
  };
  ...
  _populateBatches(); // initial populate

  // ── Generate ───────────────────────────────────────────────
  container.querySelector('#asGenBtn').addEventListener('click', () => {
    _batchId = container.querySelector('#asBatch').value;
    _generateSheet(container.querySelector('#asOutput'), _batchId);
  });
}

// ── Sheet renderer ─────────────────────────────────────────────
function _generateSheet(output, batchId) {
  if (!batchId) {
    output.innerHTML = `
      <div style="padding:40px;text-align:center;color:var(--t3);border:1px dashed var(--border2);
        ...
        <div style="font-size:13px;font-weight:600;color:var(--t2)">Select a batch to generate the sheet</div>
      </div>`;
    return;
  }
  ...
  if (!students.length) {
    output.innerHTML = `<div style="padding:32px;text-align:center;color:var(--t3);font-size:13px">
      No active enrolled students in this batch.</div>`;
    return;
  }

  // All attendance records for this batch
  const records = AppState.get('attendance') || [];
  const batchRecs = records.filter(r => r.batchId === batchId);

  // All unique dates sorted
  const dates = [...new Set(batchRecs.map(r => r.date))].sort();

  // Build map: studentId_date → status
  const recMap = {};
  ...
  const rows = students.map((stu, idx) => {
    ...
    const cells = dates.map(d => {
      ...
      return `<td style="text-align:center;padding:5px 4px;border-bottom:1px solid var(--border);
                          border-right:1px solid var(--border2);font-size:11.5px;font-weight:700;
                          color:${color}">${status || '—'}</td>`;
    }).join('');
    ...
    return `<tr style="background:${idx % 2 === 0 ? 'var(--surface)' : 'var(--surface2)'}">
      <td style="padding:6px 10px;border-bottom:1px solid var(--border);border-right:1px solid var(--border2);
                 text-align:center;color:var(--t4);font-size:11px;font-family:var(--font-mono)">${idx + 1}</td>
      <td style="padding:6px 10px;border-bottom:1px solid var(--border);border-right:1px solid var(--border2);
                 font-weight:600;color:var(--t1);white-space:nowrap">${stu.studentName || '—'}</td>
      <td style="padding:6px 10px;border-bottom:1px solid var(--border);border-right:1px solid var(--border2);
                 font-family:var(--font-mono);font-size:11px;color:var(--t3);white-space:nowrap">
        ${stu.cnic || stu.registrationNo || '—'}
      </td>
      ${cells}
      <td style="padding:6px 10px;border-bottom:1px solid var(--border);border-right:1px solid var(--border2);
                 text-align:center;font-weight:700;font-size:12px;color:var(--green)">${p}</td>
      <td style="padding:6px 10px;border-bottom:1px solid var(--border);border-right:1px solid var(--border2);
                 text-align:center;font-weight:700;font-size:12px;color:var(--red)">${a}</td>
      <td style="padding:6px 10px;border-bottom:1px solid var(--border);border-right:1px solid var(--border2);
                 text-align:center;font-weight:700;font-size:12px;color:var(--yellow)">${l}</td>
  ...
  const monthHeaders = months.map(mk => {
    ...
    return `<th colspan="${byMonth[mk].length}" style="
        ...
        background:var(--blue-dim);color:var(--blue);border-right:1px solid var(--border2);
  ...
  const summaryRow = dates.map(d => {
    ...
    return `<td style="padding:5px 4px;text-align:center;font-size:10.5px;font-weight:700;
                       ...
                       background:var(--surface3)">${pct !== null ? pct + '%' : '—'}</td>`;
  }).join('');
  ...
  output.innerHTML = `
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;flex-wrap:wrap">
      ...
      <button id="asPrintBtn" style="margin-left:auto;display:inline-flex;align-items:center;gap:6px;
          height:34px;padding:0 14px;border-radius:var(--r-sm);border:1px solid var(--border2);
    ...
    ${!dates.length
      ...
      : `<div style="overflow-x:auto;border:1px solid var(--border);border-radius:var(--r-lg)">
           <table id="asTable" style="border-collapse:collapse;font-size:12.5px;min-width:100%">
             <thead>
               <tr>
                 <th rowspan="2" style="padding:8px 10px;text-align:center;font-size:10px;font-weight:700;
                     color:var(--t3);background:var(--surface2);border-right:1px solid var(--border2);
                     border-bottom:1px solid var(--border);width:36px">#</th>
                 <th rowspan="2" style="padding:8px 10px;text-align:left;font-size:10px;font-weight:700;
                     color:var(--t3);background:var(--surface2);border-right:1px solid var(--border2);
                     border-bottom:1px solid var(--border);min-width:160px">Student Name</th>
                 <th rowspan="2" style="padding:8px 10px;text-align:left;font-size:10px;font-weight:700;
                     color:var(--t3);background:var(--surface2);border-right:1px solid var(--border2);
                     border-bottom:1px solid var(--border);min-width:120px">ID / CNIC</th>
                 ${monthHeaders}
                 <th colspan="3" style="padding:6px 8px;text-align:center;font-size:10px;font-weight:700;
                     color:var(--t3);background:var(--surface2);border-right:1px solid var(--border2);
                     border-bottom:1px solid var(--border2)">Total</th>
                 <th rowspan="2" style="padding:8px 10px;text-align:center;font-size:10px;font-weight:700;
                     color:var(--t3);background:var(--surface2);border-bottom:1px solid var(--border);
                     min-width:52px">%</th>
               </tr>
               <tr>
                 ${dateHeaders}
                 <th style="padding:5px 6px;text-align:center;font-size:10px;font-weight:700;
                     color:var(--green);background:var(--surface2);border-right:1px solid var(--border2);
                     border-bottom:1px solid var(--border);min-width:32px">P</th>
                 <th style="padding:5px 6px;text-align:center;font-size:10px;font-weight:700;
                     color:var(--red);background:var(--surface2);border-right:1px solid var(--border2);
                     border-bottom:1px solid var(--border);min-width:32px">A</th>
                 <th style="padding:5px 6px;text-align:center;font-size:10px;font-weight:700;
                     color:var(--yellow);background:var(--surface2);border-right:1px solid var(--border2);
                     border-bottom:1px solid var(--border);min-width:32px">L</th>
               </tr>
             </thead>
             <tbody>
               ${rows}
               <!-- Summary row -->
               <tr>
                 <td colspan="3" style="padding:6px 10px;font-size:11px;font-weight:700;color:var(--t2);
  ...
  output.querySelector('#asPrintBtn')?.addEventListener('click', () => {
    ...
    win.document.write(`
      <html><head><title>Attendance Sheet — ${batch?.batchName || ''}</title>
      <style>
      ...
      <body>
        ...
        <p style="color:#666;margin-bottom:12px;font-size:10px">
          ${disc?.abbreviation || ''} ${campus ? '· ' + campus.campusName : ''} · ${students.length} students · ${dates.length} days
        </p>
        ${table.outerHTML}
      </body></html>`);
    win.document.close();
